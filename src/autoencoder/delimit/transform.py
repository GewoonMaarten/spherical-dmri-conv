...
from autoencoder.delimit.utils import cart2sph, sph2cart
...
class Signal2SH(nn.Module):
    ...
    def __init__(self, sh_order, gradients, lb_lambda=0.006):
            ...
            for id_order in range(0, self.sh_order + 1, 2):
                for id_degree in range(-id_order, id_order + 1):
                    gradients_phi, gradients_theta, gradients_z = cart2sph(
                        gradients[id_gradient, 0], gradients[id_gradient, 1], gradients[id_gradient, 2]
                    )
        ...
        self.Signal2SHMat = torch.nn.Parameter(
            torch.from_numpy(np.matmul(b_inv, b.transpose()).transpose()).float(), requires_grad=False
        )
    ...
    def forward(self, x_in):
        x = x_in.reshape(
            (
                -1,
                np.ceil(x_in.size(1) / self.num_gradients).astype(int),
                self.num_gradients,
                x_in.size(2),
                x_in.size(3),
                x_in.size(4),
...
class SH2Signal(nn.Module):
    ...
    def forward(self, x_in):
        ...
        x = x_in.reshape(
            (
                x_dim[0],
                np.ceil(x_in.size(1) / self.num_coefficients).astype(int),
