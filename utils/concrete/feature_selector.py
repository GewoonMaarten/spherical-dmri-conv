...
class ConcreteAutoencoderFeatureSelector:
    ...
    def fit(self, X, val_X=None):
        ...
        steps_per_epoch = (
            X.__len__()
        )  # (len(X) + self.batch_size - 1) // self.batch_size
        ...
        for i in range(self.tryout_limit):
            ...
            for epoch in range(num_epochs):
                ...
                if val_X is not None:
                    ...
                    logger.info(
                        "epoch: %s/%s, loss: %4.4f, val loss: %4.4f",
                        epoch + 1,
                        num_epochs,
                        total_loss,
                        total_val_loss,
                ...
                logger.info(
                    "mean max of probabilities: %.8f, temperature: %.8f",
                    value_stop.item(),
                    self.model.temp.item(),
                )
                ...
                torch.save(
                    ...
                    Path(
                        ...
                        "runs",
                        "models",
                        f"checkpoint_epoch={num_epochs}_net.pth",
                    ),
                )
    ...
    def get_mask(self):
        ...
        val = torch.sum(
            nn.functional.one_hot(
                torch.argmax(self.model.logits, dim_argmax), self.model.logits.size()[1]
            )
        )
        return val
